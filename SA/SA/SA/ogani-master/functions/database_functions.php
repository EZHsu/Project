<?php
	...
	function select4LatestBook($conn){
		$row = array();
		$query = "SELECT bk_isbn, bk_img,name,bk_date FROM books ORDER BY bk_isbn DESC";
	...
	function getBookByIsbn($conn, $isbn){
		$query = "SELECT name, bk_author, bk_publish FROM books WHERE bk_isbn = '$isbn'";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
	...
	function getAll($conn){
		$query = "SELECT * from books ORDER BY bk_isbn DESC";
		$result = mysqli_query($conn, $query);
		if(!$result){
			echo "Can't retrieve data " . mysqli_error($conn);
			exit;
